...
def test_start_command_delegates_to_vm_controller(
    ...
    def fake_start(self: VmController, *, resume: bool = False) -> RunningVm:
...
def test_vm_controller_start_boots_vm_and_restarts_talon(
    ...
    monkeypatch.setattr(
        vm_module.lume,
        "spawn_vm",
        lambda vm, debug=False: calls.append(("spawn_vm", vm)) or _fake_launch(),
    )
    monkeypatch.setattr(
        vm_module.lume,
        "wait_for_running_vm",
...
def test_smoke_test_runner_failure_after_start_still_stops_vm(
    ...
    monkeypatch.setattr(
        vm_controller,
        "start",
        lambda *, resume=False: _running_vm(),
    )
    monkeypatch.setattr(
        runner,
        "_build_transfer_service",
